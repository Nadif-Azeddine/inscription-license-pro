<div class="col-12 position-relative rounded-pill bg-muted" style="height: 7px;">
    ...
    <div style="z-index: 3"
    class="circle {{ Auth::check() && Auth::user()->candidat && Auth::user()->candidat->candidature->inscriptions ? 'passed' : '' }} {{ Route::currentRouteName() === 'choix' ? 'current shadow' : '' }} ">
    ...
    <div class="position-absolute top-0 start-0 bg-info"
        style="height: 7px; z-index: 0; width:75%"></div>       
    @endif
    @if (Auth::check() && Auth::user()->candidat && Auth::user()->candidat->bac)
    <div class="position-absolute top-0 start-0 bg-info"
    style="height: 7px; z-index: 0; width:50%"></div> 
    @endif
    @if (Auth::check() && Auth::user()->candidat)
    <div class="position-absolute top-0 start-0 bg-info"
    style="height: 7px; z-index: 0; width:25%"></div> 
    @endif
